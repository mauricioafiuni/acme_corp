...
sudo echo "<html><body><h1>ACME Corp APP located at $(curl http://169.254.169.254/latest/meta-data/placement/availability-zone -H "X-aws-ec2-metadata-token: $TOKEN" | sed 's/.$//')</h1></body></html>" > /var/www/html/index.html
EOF
